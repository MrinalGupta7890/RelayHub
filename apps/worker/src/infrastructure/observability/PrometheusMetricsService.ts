...
export class PrometheusMetricsService implements MetricsService {
  ...
  constructor() {
    // Note: If using a shared registry or pushgateway, metrics registration logic 
    // might need to handle duplicate names across different processes.
    // For our pull-based setup, having same metric names in worker and api is fine
    // as prometheus labels them with different `job` names (api vs worker).
